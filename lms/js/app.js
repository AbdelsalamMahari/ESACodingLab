    ...
    if (bookName.value && authorName.value && price.value && category.value && description.value && qty.value && image.files[0]) {
        $.ajax({
            ...
            success: function (data, status) {
                ...
                showData();
                clearData();
                document.getElementById('alert').innerHTML = '<div class="alert alert-success">Book successfully added!</div>';
            },
            error: function (error) {
                console.error('Error sending data:', error);
                document.getElementById('alert').innerHTML = '<div class="alert alert-danger">Error sending data</div>';
            }
        });
    } else {
        document.getElementById('alert').innerHTML = '<div class="alert alert-danger">Please fill out all fields</div>';
    }
};
    ...
    if (bookName.value && authorName.value && price.value && category.value && description.value && qty.value && image.files[0]) {
        $.ajax({
            ...
            success: function (response) {
                ...
                document.getElementById('alert').innerHTML = '<div class="alert alert-success">Book successfully updated.</div>';
            },
            error: function (status, error) {
                console.error('Error updating data:', error);
                document.getElementById('alert').innerHTML = '<div class="alert alert-danger">Error updating data.</div>';
            }
        });
    } else {
        console.error('Please fill out all required fields.');
        document.getElementById('alert').innerHTML = '<div class="alert alert-danger">Please fill out all required fields.</div>';
    }
}
