...
<div class="col-10">
    ...
    if (mysqli_num_rows($result) == 0) {
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            echo '<td>$' . $total . '</td>';
            if ($status == 'borrow') {
                echo '<td><input type="date" class="form-control" name="borrow_time['.$bookId.']" id="date" " required></td>';
            } else {
                echo '<td><i class="fa-solid fa-slash" style="color:#FCB940;"></i><i class="fa-solid fa-slash" style="color:#FCB940;"></i></td>';
            }
            echo '<td>
        ...
        echo '<button type="" class="btn btn-primary" id="btn-primary3">Checkout</button>';
        echo '</div>';
    }
    ?>
</div>
